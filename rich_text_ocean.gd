...
func _process_custom_fx(char_fx):
	...
	var noise_seed = char_fx.env.get("seed", 0)
	
	var progress = speed * char_fx.elapsed_time + (char_fx.range.x / float(span))
	
	char_fx.color = Color.from_ok_hsl(NoiseUtils.octave_noise_1d(progress * 2.5, noise_seed ^ 123456) * 0.1 + hue, \
	 sat, 0.15 - absf(NoiseUtils.octave_noise_1d(progress * 1.6 + cos(progress), noise_seed ^ -123456789, 3)) * 0.3 + val, char_fx.color.a)
	return true
